...
public class TimeNormalizer {
    ...
    public var isPreferFuture: Bool
    ...
    func timeEx(target: String, timeBase: String) -> [TimeUnit] {
        ...
        for i in 0..<rpointer {
            ...
            unit.tpOrigin = unit.tp
            timeResult.append(unit)
        }
        
        timeResult = TimeNormalizer.filterTimeUnits(timeResult)
        return timeResult
    }
     ...
     *
     */
    static func filterTimeUnits(_ timeUnits: [TimeUnit]) -> [TimeUnit] {
        guard !timeUnits.isEmpty else {return timeUnits}
        
        var newTimeUnits: [TimeUnit] = []
        timeUnits.forEach { (timeUnit) in
            if (timeUnit.time?.timeIntervalSince1970 ?? 0) * 1000 > 0 {
                newTimeUnits.append(timeUnit)
            }
        }
